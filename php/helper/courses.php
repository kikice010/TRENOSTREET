...
class COURSES {
    ...
    public function userCoursesGet() {
        try {
        $stmt = $this->conn->db->prepare("SELECT c.id,c.id_city,c.description,c.price_monthly,c.price_weekly,c.price_yearly,c.price_hour,c.name,c.address,hc.id_category FROM courses AS c LEFT JOIN has_category AS hc ON c.id = hc.id_course WHERE c.id_trainer=:id");
